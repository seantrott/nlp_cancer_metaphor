...
```{r cars}
dat <- read.csv('../../data/processed/combined_projects.csv')
head(dat)
```
...
```{r}
ggplot(dat) + geom_bar(aes(x=year, fill=source), stat="count") + ggtitle("Number of projects per year")
```
...
```{r}
ggplot(dat) + geom_bar(aes(x=from_US, fill=source), stat="count") + ggtitle("Proportion of projects per country")
```
...
```{r}
ggplot(dat) + geom_bar(aes(x=dominant, fill=source), stat="count") + ggtitle("Dominant metaphor type count")
```
