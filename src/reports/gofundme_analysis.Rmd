---
title: "Metaphor Analysis in GoFundMe Campaigns"
output:
  pdf_document:
    toc: yes
  html_document:
...
Very few projects are in 2014, and so for simplicity's sake and to simplify the model just a tad, we remove years less than 2015.
...
**IV**: Goal amount, text_length_words, duration_float, month, year, dominant_*
...
```{r}
mod.none.corr <- glmer(status ~ (1|year) + (1|month), data = dat, family = "binomial")
...
mod.none <- glmer(status ~ (1|month) + (1|year), 
                  data = dat, family = "binomial", nAGQ = 0)

# add length of text (in words)
mod.length <- glmer(status ~ text_length_words_sc + (1|month) + (1|year), 
                              data = dat, family = "binomial", nAGQ = 0)

# add scaled goal amount
mod.length.goal <- glmer(status ~ goal_sc + text_length_words_sc + (1|month) + (1|year), 
                                   data = dat, family = "binomial", nAGQ = 0)

# add category
# mod.staff.dur.length.goal.cat <- glmer(status ~ goal_sc + text_length_words_sc + 
#                                      duration_cat + (1|month) + (1|year), 
#                                    data = dat, family = "binomial", nAGQ = 0)

# add first instantiation
mod.length.goal.inst <- glmer(status ~ goal_sc + text_length_words_sc + 
                                           first_instantiation + (1|month) + (1|year), 
                                         data = dat, family = "binomial", nAGQ = 0)

anova(mod.none, mod.length, test='Chisq')
anova(mod.length, mod.length.goal, test='Chisq')
anova(mod.length.goal, mod.length.goal.inst, test='Chisq')
```

The final base model before adding metaphor variables:

```{r}
summary(mod.length.goal.inst)
```
...
hist(dat$backers[dat$backers < 1200])
```

We limit to 600 because removing the outliers leaves us with a nicely poisson shaped distribution.

```{r}
dat.pois <- dat[dat$backers < 1200, ]
```


```{r}
mod.none.corr <- glmer(backers ~ (1|month) + (1|year), data = dat.pois, family = "poisson")
as.data.frame(VarCorr(mod.none.corr))
```
...
Priority to getting more data from GoFundMe
