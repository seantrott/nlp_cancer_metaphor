...
df = read_csv("data/processed/kickstarter_plus_metaphor.csv")
...
df$dominant_metaphor = factor(df$dominant_metaphor)
```
...
A simple OLS regression modeling log(average transaction) as a function of the dominant metaphor in a campaign suggests that **journey** campaigns have higher average gift sizes, consistent with the figure above.

```{r}
simple = lm(data = df,
            log10(avg_transaction) ~ dominant_metaphor)
summary(simple)
```
...
avg_transaction_full = lmer(data = df,
                            log10(avg_transaction) ~ dominant_metaphor + staff_pick + num_words + (1 | geo_country) + (1 | geo_state),
                    REML=FALSE)

avg_transaction_reduced = lmer(data = df,
                            log10(avg_transaction) ~  staff_pick + num_words + (1 | geo_country) + (1 | geo_state),
                            REML=FALSE)
...
df_finished$status = factor(df_finished$status)
simple = glm(data = df_finished,
             status ~ dominant_metaphor,
             family = binomial())
summary(simple)
```
