...
Null1: Journey metaphors dominating the campaign text does not significantly predict the success, compared to camapigns with only battle metaphors or fewer battle metaphors than journey metaphors.

Null2: Battle metaphors dominating the campaign text does not significantly predict the success, compared to camapigns with only journey metaphors or fewer journey metaphors than battle metaphors.

Null3: Only having the presence of journey metaphors in the campaign text does not significantly predict the success, compared to campaigns with only battle metaphors or a combination of both journey and battle metaphors.

Null4: Only having the presence of battle metaphors in the campaign text does not significantly predict the success, compared to campaigns with only journey metaphors or a combination of both journey and battle metaphors.
...
p3 = projects.jor %>%
  ggplot() + th +
  geom_violin(aes(status, journey_instantiation, fill=status)) +
  scale_fill_manual(values = c("0" = alpha("red", 0.5), "1" = alpha("blue", 0.5)), guide=F) +
  geom_hline(yintercept = mean(projects.jor[projects.jor$status == 0, ]$journey_instantiation), lty=2, color="red") +
  geom_hline(yintercept = mean(projects.jor[projects.jor$status == 1, ]$journey_instantiation), lty=2, color="blue")
...
mod = glmer(update(f,  ~ . + journey_prod), data = projects.jor, family = "binomial")
compare(status.mod, mod, "journey_prod")

mod = glmer(update(f,  ~ . + journey_instantiation), data = projects.jor, family = "binomial")
compare(status.mod, mod, "journey_instantiation")
```
...
mod = glmer(update(base.formula,  ~ . + journey_prod), data = projects.jor, family = "binomial")
compare(status.mod.base, mod, "journey_prod")

mod = glmer(update(base.formula,  ~ . + journey_instantiation), data = projects.jor, family = "binomial")
compare(status.mod.base, mod, "journey_instantiation")
```
...
p2 = projects.bat %>%
  ...
  scale_fill_manual(values = c("0" = alpha("red", 0.5), "1" = alpha("blue", 0.5)), guide=F) +
  geom_hline(yintercept = mean(projects.bat[projects.bat$status == 0, ]$battle_prod), lty=2, color="red") +
  geom_hline(yintercept = mean(projects.bat[projects.bat$status == 1, ]$battle_prod), lty=2, color="blue")

p3 = projects.bat %>%
  ggplot() + th +
  geom_violin(aes(status, battle_instantiation, fill=status)) +
  scale_fill_manual(values = c("0" = alpha("red", 0.5), "1" = alpha("blue", 0.5)), guide=F) +
  geom_hline(yintercept = mean(projects.bat[projects.bat$status == 0, ]$battle_instantiation), lty=2, color="red") +
  geom_hline(yintercept = mean(projects.bat[projects.bat$status == 1, ]$battle_instantiation), lty=2, color="blue")
...
mod = glmer(update(f,  ~ . + battle_prod), data = projects.bat, family = "binomial")
compare(status.mod, mod, "battle_prod")

mod = glmer(update(f,  ~ . + battle_instantiation), data = projects.bat, family = "binomial")
compare(status.mod, mod, "battle_instantiation")
```
...
mod = glmer(update(base.formula,  ~ . + battle_prod), data = projects.bat, family = "binomial")
compare(status.mod.base, mod, "battle_prod")

mod = glmer(update(base.formula,  ~ . + battle_instantiation), data = projects.bat, family = "binomial")
compare(status.mod.base, mod, "battle_instantiation")
```
