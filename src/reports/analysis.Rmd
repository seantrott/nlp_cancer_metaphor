...
dg <- dat %>%
  group_by(category) %>%
  summarise(
    battle_sal_mean = mean(battle_salience),
    journey_sal_mean = mean(journey_salience)
  )

dgm <- melt(dg, experiment=c("Battle", "Journey"), variable.name="Type")

ggplot(dgm, aes(category, value, fill=Type)) +
...
```{r}
dat[dat$text_length_words == 0, c("mean_donation", "backers", "dominant")]
```
...
mod.dom.battle.sal <- glmer(status ~ battle_salience + dominant_battle + category + goal_sc + text_length_words_sc +
                          duration_cat + staff_pick + (1|geo_state) + (1|month) + (1|year), 
                        data = dat, 
                        family = "binomial", 
                        nAGQ = 0)
...
mod.dom.journey.sal <- glmer(status ~ journey_salience + dominant_journey + category + goal_sc + text_length_words_sc +
                           duration_cat + staff_pick + (1|geo_state) + (1|month) + (1|year),
                         data = dat,
                         family = "binomial",
                         nAGQ = 0)
...
anova(mod.dom.battle, mod.dom.battle.sal)
anova(mod.staff.dur.length.goal.cat, mod.dom.journey)
anova(mod.dom.journey, mod.dom.journey.sal)
