...
Null1: Journey metaphors dominating the campaign text does not significantly predict the total number of donors, compared to camapigns with only battle metaphors or fewer battle metaphors than journey metaphors.

Null2: Battle metaphors dominating the campaign text does not significantly predict the total number of donors, compared to camapigns with only journey metaphors or fewer journey metaphors than battle metaphors.

Null3: Only having the presence of journey metaphors in the campaign text does not significantly predict the total number of donorsdonation, compared to campaigns with only battle metaphors or a combination of both journey and battle metaphors.

Null4: Only having the presence of battle metaphors in the campaign text does not significantly predict the total number of donors, compared to campaigns with only journey metaphors or a combination of both journey and battle metaphors.
...
p1 = projects.some %>%
  ...
  scale_fill_manual(values = c("FALSE" = alpha("red", 0.5), "TRUE" = alpha("blue", 0.5)), guide=F) + 
  geom_hline(yintercept = mean(projects.some[projects.some$dom_journey == F, ]$backers), lty=2, color="red") + 
  geom_hline(yintercept = mean(projects.some[projects.some$dom_journey, ]$backers), lty=2, color="blue")

p2 = projects.some %>%
  ggplot() + theme_minimal() +
  geom_violin(aes(dom_battle, backers, fill=dom_battle)) + 
  scale_fill_manual(values = c("FALSE" = alpha("red", 0.5), "TRUE" = alpha("blue", 0.5)), guide=F) + 
  geom_hline(yintercept = mean(projects.some[projects.some$dom_battle == F, ]$backers), lty=2, color="red") + 
  geom_hline(yintercept = mean(projects.some[projects.some$dom_battle, ]$backers), lty=2, color="blue")

p3 = projects.some %>%
  ggplot() + theme_minimal() +
  geom_violin(aes(only_journey, backers, fill=only_journey)) + 
  scale_fill_manual(values = c("FALSE" = alpha("red", 0.5), "TRUE" = alpha("blue", 0.5)), guide=F) + 
  geom_hline(yintercept = mean(projects.some[projects.some$only_journey == F, ]$backers), lty=2, color="red") + 
  geom_hline(yintercept = mean(projects.some[projects.some$only_journey, ]$backers), lty=2, color="blue")

p4 = projects.some %>%
  ggplot() + theme_minimal() +
  geom_violin(aes(only_battle, backers, fill=only_battle)) + 
  scale_fill_manual(values = c("FALSE" = alpha("red", 0.5), "TRUE" = alpha("blue", 0.5)), guide=F) + 
  geom_hline(yintercept = mean(projects.some[projects.some$only_battle == F, ]$backers), lty=2, color="red") + 
  geom_hline(yintercept = mean(projects.some[projects.some$only_battle, ]$backers), lty=2, color="blue")
...
```{r}
backers.mod.base = glmmTMB(base.formula, data = projects.some, family = "truncated_nbinom2")
# drop1(backers.mod.base, test="Chisq")
```

```{r}
f = update(base.formula,  ~ . - month)
# drop1(glmmTMB(f, data = projects.some, family = "truncated_nbinom2"), test="Chisq")
```
...
```{r}
backers.mod = glmmTMB(f, data = projects.some, family = "truncated_nbinom2")
# summary(backers.mod)
```
...
```{r}
2*(logLik(mod) - logLik(backers.mod))
```

```{r}
anova(backers.mod, mod)
```



```{r some metaphors reduced}
mod = glmmTMB(update(f,  ~ . + dom_journey), data = projects.some, family = "truncated_nbinom2")
compare(backers.mod, mod, "dom_journeyTRUE")

mod = glmmTMB(update(f,  ~ . + dom_battle), data = projects.some, family = "truncated_nbinom2")
compare(backers.mod, mod, "dom_battleTRUE")

mod = glmmTMB(update(f,  ~ . + only_journey), data = projects.some, family = "truncated_nbinom2")
compare(backers.mod, mod, "only_journeyTRUE")

mod = glmmTMB(update(f,  ~ . + only_battle), data = projects.some, family = "truncated_nbinom2")
compare(backers.mod, mod, "only_battleTRUE")
```

```{r some metaphors full}
mod = glmmTMB(update(base.formula,  ~ . + dom_journey), data = projects.some, family = "truncated_nbinom2")
compare(backers.mod.base, mod, "dom_journeyTRUE")

mod = glmmTMB(update(base.formula,  ~ . + dom_battle), data = projects.some, family = "truncated_nbinom2")
compare(backers.mod.base, mod, "dom_battleTRUE")

mod = glmmTMB(update(base.formula,  ~ . + only_journey), data = projects.some, family = "truncated_nbinom2")
compare(backers.mod.base, mod, "only_journeyTRUE")

mod = glmmTMB(update(base.formula,  ~ . + only_battle), data = projects.some, family = "truncated_nbinom2")
...
p2 = projects.jor %>%
  ggplot(aes(journey_prod, backers)) + theme_minimal() +
  geom_point() + geom_smooth(method = "lm", se = F, color="red") + geom_smooth(method = "lm", formula = y ~ poly(x, 2), se = F, color="blue")

p3 = projects.jor %>%
  ggplot(aes(journey_instantiation, backers)) + theme_minimal() +
  geom_point() + geom_smooth(method = "lm", se = F, color="red") + geom_smooth(method = "lm", formula = y ~ poly(x, 2), se = F, color="blue")
...
mod = glmmTMB(update(f,  ~ . + journey_prod), data = projects.jor, family = "truncated_nbinom2")
compare(backers.mod, mod, "journey_prod")

mod = glmmTMB(update(f,  ~ . + journey_instantiation), data = projects.jor, family = "truncated_nbinom2")
compare(backers.mod, mod, "journey_instantiation")
```
...
mod = glmmTMB(update(base.formula,  ~ . + journey_prod), data = projects.jor, family = "truncated_nbinom2")
compare(backers.mod.base, mod, "journey_prod")

mod = glmmTMB(update(base.formula,  ~ . + journey_instantiation), data = projects.jor, family = "truncated_nbinom2")
compare(backers.mod.base, mod, "journey_instantiation")
```
...
p2 = projects.bat %>%
  ggplot(aes(battle_prod, backers)) + theme_minimal() +
  geom_point() + geom_smooth(method = "lm", se = F, color="red") + geom_smooth(method = "lm", formula = y ~ poly(x, 2), se = F, color="blue")

p3 = projects.bat %>%
  ggplot(aes(battle_instantiation, backers)) + theme_minimal() +
  geom_point() + geom_smooth(method = "lm", se = F, color="red") + geom_smooth(method = "lm", formula = y ~ poly(x, 2), se = F, color="blue")
...
mod = glmmTMB(update(f,  ~ . + battle_prod), data = projects.bat, family = "truncated_nbinom2")
compare(backers.mod, mod, "battle_prod")

mod = glmmTMB(update(f,  ~ . + battle_instantiation), data = projects.bat, family = "truncated_nbinom2")
compare(backers.mod, mod, "battle_instantiation")
```
...
mod = glmmTMB(update(base.formula,  ~ . + battle_prod), data = projects.bat, family = "truncated_nbinom2")
compare(backers.mod.base, mod, "battle_prod")

mod = glmmTMB(update(base.formula,  ~ . + battle_instantiation), data = projects.bat, family = "truncated_nbinom2")
compare(backers.mod.base, mod, "battle_instantiation")
```
