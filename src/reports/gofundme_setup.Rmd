...
```{r}
mean(dat$duration_float)
sd(dat$duration_float)

mean(dat$any_metaphor)
sd(dat$any_metaphor)
```

```{r}
ggplot(dat) +
  geom_histogram(aes(0.5*(duration_float - mean(dat$duration_float)) / sd(dat$duration_float)), fill="blue", alpha=0.5) + 
  geom_histogram(aes(scale(duration_float)), fill="red", alpha=0.5) + 
  geom_histogram(aes)
# hist((dat$duration_float - mean(dat$duration_float)) / (2*sd(dat$duration_float)))
```
