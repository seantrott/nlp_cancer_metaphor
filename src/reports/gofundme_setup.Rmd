...
```{r}
mean(dat$journey_metaphor)
mean(dat$journey_salience)
```
...
```{r}
table(labs$keyword, labs$metaphorical)
```
...
```{r}
ggplot() + 
  geom_density(data = dat[dat$only_journey == T, ], aes(first_instantiation), fill=rgb(1,0.5,0.5), alpha=0.3) +
  geom_density(data = dat[dat$only_battle == T, ], aes(first_instantiation), fill=rgb(0.5,0.5,1), alpha=0.3) +
  geom_density(data = dat[dat$dom_battle == T, ], aes(first_instantiation), fill=rgb(0,0,1), alpha=0.3) +
  geom_density(data = dat[dat$dom_journey == T, ], aes(first_instantiation), fill=rgb(1,0,0), alpha=0.3) +
