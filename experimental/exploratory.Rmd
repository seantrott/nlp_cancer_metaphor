...
```{r}
t = table(trials[, c("self_cancer", "ff_cancer")])
t = cbind(t, rowSums(t))
colnames(t)[4] = "Total"
t = rbind(t, colSums(t))
rownames(t)[4] = "Total"
...
```{r}
t = table(trials$education)
t["total"] = nrow(trials)
t
```

Socioeconomics of participants

```{r}
t = table(trials$socioeconomic)
t["total"] = nrow(trials)
t
```
...
First, 5th, 50th, and 95th quantiles for the actual trial response time in seconds. Then the mean trial response time. Then 5th, 50th, and 95th quantiles for experiment total duration in minutes. Lastly, mean total experiment duration.
...
```{r}
data %>%
  tidyr::gather(question, value, urgent:sympathy) %>%
...
```{r}
model.base = lm(update(base.formula, . ~ . + cond_sex), data = trials) # gender is already included in model
...
trials = mutate(trials, 
                gender_congruent = (gender == "Male" & cond_sex == "male") | (gender == "Female" & cond_sex == "female"))
...
TODO: Responses will be coded for the presence of metaphor, either mentioning Only Journey, Only Battle, Mixed Metaphors, or No Metaphors. Perhaps to be done via Python, similar to methods of annotating GoFundMe corpus.
...
TODO: cross-reference with MTurk IDs
