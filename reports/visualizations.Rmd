...
p2 = projects %>%
  ggplot() + t + labs(title = "Success", x="", y="Count") +
  geom_bar(aes(status, fill="a"), width = 0.5) + scale_x_discrete(labels = c("0"="Failure", "1"="Success")) +
  geom_text(data = d, aes(x=x, y = y, label=vals), color=alpha("white", 1), show.legend = F, family = "Times", size=3) +
...
ggsave("paper_dvs.png", p, width = 3.54, height = 4.75)
```
